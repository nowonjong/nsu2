...
TTS_DEBUG = env_flag("NSU_RUN_TTS_DEBUG", True)
...
TTS_EDGE_PRECACHE = env_flag("NSU_RUN_TTS_EDGE_PRECACHE", True)
...
def get_edge_tts_path(text):
    ...
    if not clean_text:
        return None
    if clean_text in edge_tts_cache and os.path.exists(edge_tts_cache[clean_text]):
        return edge_tts_cache[clean_text]

    import edge_tts

    safe_name = base64.urlsafe_b64encode(clean_text.encode("utf-8")).decode("ascii").rstrip("=")
    output_path = os.path.join(tempfile.gettempdir(), f"nsu_tts_{safe_name}.mp3")
    ...
    edge_tts_cache[clean_text] = output_path
    return output_path


def play_audio_file_windows(path):
...
def speak_with_edge_tts(text):
    ...
    if audio_path:
        play_audio_file_windows(audio_path)
...
def tts_worker():
    ...
    if backend == "edge":
        try:
            ...
            if TTS_DEBUG:
                print(f"[TTS] Edge neural backend ready voice={TTS_EDGE_VOICE} rate={TTS_EDGE_RATE}")
        except Exception as e:
            print(f"[TTS] edge-tts unavailable, using Windows SAPI fallback: {e}")
            backend = "sapi"
