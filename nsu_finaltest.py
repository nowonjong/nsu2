...
from tensorflow_gpu_bootstrap import configure_tensorflow_gpu_dlls

GPU_DLL_DIRS = configure_tensorflow_gpu_dlls()
...
DEFAULT_VISION_MODEL_DIR = os.getenv(
    "NSU_RUN_VISION_MODEL_DIR",
    os.path.join(os.path.dirname(__file__), "models", "final_run", "vision_only_seed43"),
)
VISION_MODEL_PATH = os.getenv("NSU_RUN_VISION_MODEL_PATH", os.path.join(DEFAULT_VISION_MODEL_DIR, "best_lstm.keras"))
VISION_SCALER_PATH = os.getenv("NSU_RUN_VISION_SCALER_PATH", os.path.join(DEFAULT_VISION_MODEL_DIR, "vision_scaler.npz"))
VISION_CLASS_NAMES_PATH = os.getenv("NSU_RUN_VISION_LABELS_PATH", os.path.join(DEFAULT_VISION_MODEL_DIR, "class_names.json"))
VISION_CONFIG_PATH = os.getenv("NSU_RUN_VISION_CONFIG_PATH", os.path.join(DEFAULT_VISION_MODEL_DIR, "config.json"))
SAVE_DEBUG_CAPTURE = env_flag("NSU_RUN_SAVE_DEBUG_CAPTURE", False)
...
# 시리얼 설정
# =========================
SERIAL_PORT = os.getenv("NSU_RUN_SERIAL_PORT", "COM5")
SERIAL_BAUD = env_int("NSU_RUN_SERIAL_BAUD", 115200)
...
TTS_ENABLED = env_flag("NSU_RUN_TTS", True)
TTS_MIN_INTERVAL_SEC = env_float("NSU_RUN_TTS_MIN_INTERVAL_SEC", 1.2)
TTS_SKIP_NONE = env_flag("NSU_RUN_TTS_SKIP_NONE", True)
TTS_BACKEND = os.getenv("NSU_RUN_TTS_BACKEND", "edge").strip().lower()
TTS_RATE = env_int("NSU_RUN_TTS_RATE", 0)
TTS_VOLUME = env_int("NSU_RUN_TTS_VOLUME", 100)
TTS_DEBUG = env_flag("NSU_RUN_TTS_DEBUG", False)
TTS_EDGE_VOICE = os.getenv("NSU_RUN_TTS_EDGE_VOICE", "ko-KR-SunHiNeural")
TTS_EDGE_RATE = os.getenv("NSU_RUN_TTS_EDGE_RATE", "+0%").strip()
TTS_EDGE_EXTRA_SLEEP_MS = env_int("NSU_RUN_TTS_EDGE_EXTRA_SLEEP_MS", 180)
TTS_EDGE_PRECACHE = env_flag("NSU_RUN_TTS_EDGE_PRECACHE", False)
TTS_EDGE_REQUIRE = env_flag("NSU_RUN_TTS_EDGE_REQUIRE", True)
TTS_EDGE_EXE = os.getenv("NSU_RUN_TTS_EDGE_EXE", "").strip()
DASHBOARD_WIDTH = env_int("NSU_RUN_DASHBOARD_WIDTH", 1280)
DASHBOARD_HEIGHT = env_int("NSU_RUN_DASHBOARD_HEIGHT", 720)
PLOT_HISTORY_LEN = env_int("NSU_RUN_PLOT_HISTORY", 120)
WINDOW_NAME = "Fusion Real-time Sign Recognition v5"
RUN_FULLSCREEN = env_flag("NSU_RUN_FULLSCREEN", True)
LOGO_IMAGE_PATH = os.getenv("NSU_RUN_LOGO_IMAGE_PATH", r"nsu_logo.png")
INTRO_SPLASH_SEC = env_float("NSU_RUN_INTRO_SPLASH_SEC", 2.2)
UI_LOGO_IMG = None
SHOW_RUN_LOGO = env_flag("NSU_RUN_SHOW_LOGO", False)
...
def get_kor_label_text(label):
    key = str(label).strip().lower()
    return KOR_MAP.get(key, key)
...
# TensorFlow GPU 설정
# =========================
print("TensorFlow GPU DLL dirs:", GPU_DLL_DIRS if GPU_DLL_DIRS else "not configured")
...
VISION_COMPARE_ENABLED = env_flag("NSU_RUN_VISION_COMPARE", True)
vision_model = None
VISION_ACTIONS = []
vision_only_scaler_mean = np.zeros((VISION_DIM,), dtype=np.float32)
vision_only_scaler_scale = np.ones((VISION_DIM,), dtype=np.float32)

if VISION_COMPARE_ENABLED:
    vision_required_paths = [
        VISION_MODEL_PATH,
        VISION_SCALER_PATH,
        VISION_CLASS_NAMES_PATH,
    ]
    missing_vision_paths = [path for path in vision_required_paths if not os.path.exists(path)]
    if missing_vision_paths:
        VISION_COMPARE_ENABLED = False
        print(f"[VisionCompare] disabled: missing files={missing_vision_paths}")
    else:
        vision_model = tf.keras.models.load_model(VISION_MODEL_PATH)
        with open(VISION_CLASS_NAMES_PATH, 'r', encoding='utf-8') as f:
            VISION_ACTIONS = json.load(f)
        vision_scaler_data = np.load(VISION_SCALER_PATH)
        vision_only_scaler_mean = vision_scaler_data['mean'].astype(np.float32)
        vision_only_scaler_scale = vision_scaler_data['scale'].astype(np.float32)
        vision_only_scaler_scale = np.where(
            np.abs(vision_only_scaler_scale) < 1e-8,
            1.0,
            vision_only_scaler_scale,
        ).astype(np.float32)
        if VISION_ACTIONS != ACTIONS:
            print("[VisionCompare] warning: fusion labels and vision-only labels differ")
        print(f"[VisionCompare] enabled: {VISION_MODEL_PATH}")
...
def draw_text_unicode_fit(img, text, org, max_width, font_size=40, min_font_size=22, text_color=(0, 0, 0)):
    if not text:
        return
    size = int(font_size)
    while size > min_font_size:
        bbox = get_ui_font(size).getbbox(text)
        if bbox[2] - bbox[0] <= max_width:
            break
        size -= 2
    draw_text_unicode(img, text, org, font_size=size, text_color=text_color)
...
def fit_frame_to_cover(frame, box_w, box_h):
    h, w = frame.shape[:2]
    scale = max(box_w / max(1, w), box_h / max(1, h))
    new_w = max(1, int(w * scale))
    new_h = max(1, int(h * scale))
...
def load_optional_image(path):
    ...
    if not os.path.exists(path):
        print(f"[UI] optional image not found: {path}")
        return None
    img = cv2.imread(path, cv2.IMREAD_COLOR)
    if img is None:
        print(f"[UI] failed to load optional image: {path}")
    return img
...
def load_nsu_logo():
    ...
    if not os.path.exists(LOGO_IMAGE_PATH):
        print(f"[UI] optional logo not found: {LOGO_IMAGE_PATH}")
        return None
    img = cv2.imread(LOGO_IMAGE_PATH)
    if img is None:
        print(f"[UI] failed to load optional logo: {LOGO_IMAGE_PATH}")
    return img
...
def standardize_vision_only(seq_vision):
    seq_vision = np.asarray(seq_vision, dtype=np.float32)
    seq_vision_scaled = np.zeros_like(seq_vision, dtype=np.float32)
    nonzero_mask = np.any(np.abs(seq_vision) > 1e-6, axis=1)
    if np.any(nonzero_mask):
        seq_vision_scaled[nonzero_mask] = (
            (seq_vision[nonzero_mask] - vision_only_scaler_mean) / vision_only_scaler_scale
        ).astype(np.float32)
    return seq_vision_scaled
...
def make_empty_prediction(long_motion=0.0, hand_count=0):
    return {
        'label': 'none',
        'conf': 1.0,
        'raw_label': 'none',
...
def summarize_probs(y_prob, labels):
    if len(labels) == 0:
        return make_empty_prediction()

    sorted_idx = np.argsort(y_prob)
    top1_idx = int(sorted_idx[-1])
    top2_idx = int(sorted_idx[-2]) if len(sorted_idx) > 1 else top1_idx
    top1_label = labels[top1_idx]
    top1_conf = float(y_prob[top1_idx])
    top2_conf = float(y_prob[top2_idx]) if top2_idx != top1_idx else 0.0
...
def classify_vision_sequence_once(seq_vision, long_motion, hand_count):
    result = make_empty_prediction(long_motion, hand_count)
    if not VISION_COMPARE_ENABLED or vision_model is None:
    ...
    seq_vision_scaled = standardize_vision_only(seq_vision)
    vision_input = tf.convert_to_tensor(
        np.expand_dims(seq_vision_scaled, axis=0), dtype=tf.float32
    )
    runtime_profiler.add_duration("vision_compare_preprocess", prep_start)
    infer_start = time.perf_counter()
    y_prob = infer_vision_only(vision_input).numpy()[0]
    runtime_profiler.add_duration("vision_compare_inference", infer_start)
    result.update(summarize_probs(y_prob, VISION_ACTIONS))
    result['enabled'] = True
    result['long_motion'] = long_motion
...
def classify_sequence_once(seq_array):
    ...
    result = make_empty_prediction(long_motion, hand_count)
    result['vision'] = classify_vision_sequence_once(seq_vision, long_motion, hand_count)
    ...
    fusion_summary = summarize_probs(y_prob, ACTIONS)
    fusion_summary['long_motion'] = long_motion
    fusion_summary['hand_count'] = hand_count
    fusion_summary['vision'] = result['vision']
...
def render_dashboard(
    ...
    stable_kor = get_kor_label_text(stable_label)
    vision_label = vision_result.get('label', 'none') if vision_result else 'none'
    vision_conf = float(vision_result.get('conf', 0.0)) if vision_result else 0.0
    vision_enabled = bool(vision_result.get('enabled', False)) if vision_result else False
    vision_kor = get_kor_label_text(vision_label)
    result_color = (30, 110, 190) if stable_label != 'none' else (40, 40, 40)
    draw_text_box(dashboard, (off_x + 12, off_y + 12), (off_x + 360, off_y + 84), fill_color=(255, 255, 255), alpha=0.82)
    draw_text_unicode(dashboard, "실시간 수어 인식", (off_x + 20, off_y + 18), font_size=21, text_color=(90, 90, 90))
    draw_text_unicode(dashboard, f"현재 번역: {stable_kor}", (off_x + 20, off_y + 45), font_size=28, text_color=result_color)
    ...
    put("RESULTS", result_y, scale=0.55, thick=2)
    result_box_y1 = result_y + 10
    result_box_y2 = result_box_y1 + 82
    ...
    cv2.line(dashboard, (side_x1 + 14, result_box_y1 + 41), (side_x2 - 14, result_box_y1 + 41), (230, 234, 240), 1)

    result_text_color = result_color if stable_label != 'none' else (70, 74, 82)
    vision_text_color = (70, 90, 120) if vision_enabled and vision_label != 'none' else (95, 100, 108)

    put("FUSION", result_box_y1 + 18, x_offset=24, scale=0.44, color=(80, 84, 92), thick=2)
    draw_text_unicode(dashboard, stable_kor, (side_x1 + 105, result_box_y1 + 6), font_size=23, text_color=result_text_color)
    put(f"{stable_conf * 100:.1f}%", result_box_y1 + 27, x_offset=sidebar_w - 76, scale=0.55, color=result_text_color, thick=2)

    put("VISION", result_box_y1 + 59, x_offset=24, scale=0.44, color=(80, 84, 92), thick=2)
    if vision_enabled:
        draw_text_unicode(dashboard, vision_kor, (side_x1 + 105, result_box_y1 + 47), font_size=23, text_color=vision_text_color)
        put(f"{vision_conf * 100:.1f}%", result_box_y1 + 68, x_offset=sidebar_w - 76, scale=0.55, color=vision_text_color, thick=2)
    else:
        put("off", result_box_y1 + 68, x_offset=105, scale=0.55, color=vision_text_color, thick=2)
    ...
    for idx, cand in enumerate(top_candidates[:3]):
        ...
        c_kor = get_kor_label_text(c_label)
    ...
    put("KEYS  S/C/D/P/I/Q", seq_y + 38, scale=0.48, color=(70, 74, 82), thick=2)
...
def draw_live_overlay(
    ...
    vision_enabled = bool(vision_result.get('enabled', False)) if vision_result else False
    vision_kor = get_kor_label_text(vision_label)
    mode_txt = {"WAIT": "WAIT", "COUNTDOWN": "READY", "RECORDING": "REC"}.get(mode, mode)

    h, w = img.shape[:2]
    draw_text_box(img, (0, 0), (w, 118), fill_color=(0, 0, 0), alpha=0.48)
    draw_text_unicode(img, f"FUSION  {stable_kor}  {stable_conf * 100:.1f}%", (24, 18), font_size=32, text_color=(255, 255, 255))
    if vision_enabled:
        draw_text_unicode(img, f"VISION  {vision_kor}  {vision_conf * 100:.1f}%", (24, 64), font_size=26, text_color=(210, 235, 255))
    else:
        cv2.putText(img, "VISION  OFF", (24, 92), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (210, 235, 255), 2, cv2.LINE_AA)

    status = (
        f"{mode_txt} | FPS {fps_smooth:.1f} | BUF {sequence_len}/{SEQ_LEN} | "
        f"L {int(left_seen)} R {int(right_seen)} | SD {last_sensor_host_delta_ms:.0f}ms"
    )
    if overlap_now:
        status += " | OVERLAP"
    cv2.putText(img, status, (24, h - 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 2, cv2.LINE_AA)

    if mode == "WAIT":
        cv2.putText(img, "S: START   C: RESET   Q: QUIT", (w - 380, h - 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (230, 230, 230), 2, cv2.LINE_AA)


def draw_value_bar(img, x, y, w, h, value, vmin, vmax, color, label):
    cv2.putText(img, label, (x, y + h - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.43, (70, 74, 82), 1, cv2.LINE_AA)
    bar_x = x + 48
    bar_w = max(1, w - 92)
    cv2.rectangle(img, (bar_x, y), (bar_x + bar_w, y + h), (222, 226, 232), -1)
    denom = max(vmax - vmin, 1e-6)
    ratio = float(np.clip((value - vmin) / denom, 0.0, 1.0))
    fill_w = int((bar_w - 2) * ratio)
    if fill_w > 0:
        cv2.rectangle(img, (bar_x + 1, y + 1), (bar_x + 1 + fill_w, y + h - 1), color, -1)
    cv2.rectangle(img, (bar_x, y), (bar_x + bar_w, y + h), (190, 196, 205), 1)
...
def render_showcase_layout(
    ...
    panel = (255, 255, 255)
    line = (205, 214, 224)
    canvas = np.full((DASHBOARD_HEIGHT, DASHBOARD_WIDTH, 3), pale, dtype=np.uint8)
    border = 3
    right_w = int(DASHBOARD_WIDTH * 0.24)
    top_h = int(DASHBOARD_HEIGHT * 0.18)
    left_w = DASHBOARD_WIDTH - right_w
    camera_h = DASHBOARD_HEIGHT - top_h

    result_rect = (0, 0, left_w, top_h)
    camera_rect = (0, top_h, left_w, DASHBOARD_HEIGHT)
    sensor_rect = (left_w, 0, DASHBOARD_WIDTH, DASHBOARD_HEIGHT)

    cv2.rectangle(canvas, (result_rect[0], result_rect[1]), (result_rect[2] - 1, result_rect[3] - 1), panel, -1)
    cv2.rectangle(canvas, (camera_rect[0], camera_rect[1]), (camera_rect[2] - 1, camera_rect[3] - 1), (13, 16, 20), -1)
    cv2.rectangle(canvas, (sensor_rect[0], sensor_rect[1]), (sensor_rect[2] - 1, sensor_rect[3] - 1), pale, -1)
    cv2.rectangle(canvas, (0, 0), (left_w, 8), navy, -1)
    cv2.rectangle(canvas, (left_w, 0), (DASHBOARD_WIDTH, 8), teal, -1)
    cv2.line(canvas, (0, top_h), (left_w, top_h), (18, 22, 28), border)
    cv2.line(canvas, (left_w, 0), (left_w, DASHBOARD_HEIGHT), (18, 22, 28), border)
    cv2.rectangle(canvas, (0, 0), (DASHBOARD_WIDTH - 1, DASHBOARD_HEIGHT - 1), (18, 22, 28), border)

    cam_pad = 8
    fitted = fit_frame_to_box(camera_img, left_w - cam_pad * 2, camera_h - cam_pad * 2)
    fh, fw = fitted.shape[:2]
    cam_x = cam_pad + (left_w - cam_pad * 2 - fw) // 2
    cam_y = top_h + cam_pad + (camera_h - cam_pad * 2 - fh) // 2
    cv2.rectangle(canvas, (cam_pad, top_h + cam_pad), (left_w - cam_pad - 1, DASHBOARD_HEIGHT - cam_pad - 1), (4, 6, 9), -1)
    canvas[cam_y:cam_y + fh, cam_x:cam_x + fw] = fitted
    cv2.rectangle(canvas, (cam_x, cam_y), (cam_x + fw, cam_y + fh), (52, 60, 72), 1)
    ...
    vision_enabled = bool(vision_result.get('enabled', False)) if vision_result else False
    vision_kor = get_kor_label_text(vision_label)
    mode_txt = {"WAIT": "WAIT", "COUNTDOWN": "READY", "RECORDING": "RECORDING"}.get(mode, mode)

    reserved_logo_w = 0
    if SHOW_RUN_LOGO and logo_img is not None:
        logo_size = max(64, top_h - 48)
        fitted_logo = fit_frame_to_box(logo_img, logo_size, logo_size)
        lh, lw = fitted_logo.shape[:2]
        lx = left_w - lw - 34
        ly = (top_h - lh) // 2
        cv2.rectangle(canvas, (lx - 8, ly - 8), (lx + lw + 8, ly + lh + 8), (255, 255, 255), -1)
        canvas[ly:ly + lh, lx:lx + lw] = fitted_logo
        reserved_logo_w = lw + 70

    label_x = 20
    value_x = 128
    conf_right_x = left_w - reserved_logo_w - 120
    cv2.rectangle(canvas, (label_x, 20), (label_x + 92, 52), navy, -1)
    cv2.putText(canvas, "FUSION", (label_x + 12, 42), cv2.FONT_HERSHEY_SIMPLEX, 0.57, (255, 255, 255), 2, cv2.LINE_AA)
    draw_text_unicode(canvas, stable_kor, (value_x, 15), font_size=34, text_color=(16, 20, 26))
    cv2.putText(canvas, f"{stable_conf * 100:.0f}%", (conf_right_x, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.78, navy, 2, cv2.LINE_AA)
    if vision_enabled:
        cv2.rectangle(canvas, (label_x, 72), (label_x + 92, 102), teal, -1)
        cv2.putText(canvas, "VISION", (label_x + 12, 93), cv2.FONT_HERSHEY_SIMPLEX, 0.54, (255, 255, 255), 2, cv2.LINE_AA)
        draw_text_unicode(canvas, vision_kor, (value_x, 65), font_size=28, text_color=(132, 109, 0))
        cv2.putText(canvas, f"{vision_conf * 100:.0f}%", (conf_right_x + 10, 94), cv2.FONT_HERSHEY_SIMPLEX, 0.66, (132, 109, 0), 2, cv2.LINE_AA)
    else:
        cv2.putText(canvas, "VISION OFF", (label_x, 96), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (70, 84, 105), 2, cv2.LINE_AA)
    cv2.putText(canvas, f"{mode_txt}  FPS {fps_smooth:.1f}  BUF {sequence_len}/{SEQ_LEN}", (left_w - 260, top_h - 18), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (78, 84, 96), 1, cv2.LINE_AA)

    sx = left_w + 18
    sy = 26
    panel_w = right_w - 36
    scale_y = DASHBOARD_HEIGHT / 720.0
    cv2.putText(canvas, "SENSOR STATUS", (sx, sy + 18), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (16, 20, 26), 2, cv2.LINE_AA)
    status_y1 = sy + 38
    status_h = 92
    cv2.rectangle(canvas, (sx, status_y1), (sx + panel_w, status_y1 + status_h), panel, -1)
    cv2.rectangle(canvas, (sx, status_y1), (sx + 10, status_y1 + status_h), navy, -1)
    cv2.rectangle(canvas, (sx, status_y1), (sx + panel_w, status_y1 + status_h), line, 1)
    glove_status = "GLOVE ACTIVE" if sensor_packet_count > 0 else "GLOVE WAIT"
    glove_color = (45, 130, 78) if sensor_packet_count > 0 else (170, 80, 80)
    cv2.putText(canvas, glove_status, (sx + 18, status_y1 + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.54, glove_color, 1, cv2.LINE_AA)
    cv2.putText(canvas, f"DELAY {last_sensor_host_delta_ms:.0f}ms", (sx + 18, status_y1 + 55), cv2.FONT_HERSHEY_SIMPLEX, 0.54, (35, 40, 48), 1, cv2.LINE_AA)
    cv2.putText(canvas, f"HANDS L{int(left_seen)} R{int(right_seen)}", (sx + 18, status_y1 + 82), cv2.FONT_HERSHEY_SIMPLEX, 0.54, (35, 40, 48), 1, cv2.LINE_AA)
    if overlap_now:
        cv2.putText(canvas, "OVERLAP", (sx + panel_w - 120, status_y1 + 100), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0, 90, 210), 2, cv2.LINE_AA)

    flex_y = status_y1 + status_h + 22
    footer_space = 34
    gap = 18
    imu_h = 146
    flex_h = max(230, DASHBOARD_HEIGHT - flex_y - imu_h - gap - footer_space)
    cv2.rectangle(canvas, (sx, flex_y), (sx + panel_w, flex_y + flex_h), panel, -1)
    cv2.rectangle(canvas, (sx, flex_y), (sx + 8, flex_y + flex_h), teal, -1)
    cv2.rectangle(canvas, (sx, flex_y), (sx + panel_w, flex_y + flex_h), line, 1)
    cv2.putText(canvas, "FLEX", (sx + 18, flex_y + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (16, 20, 26), 2, cv2.LINE_AA)
    bar_y = flex_y + 52
    bar_gap = max(24, int((flex_h - 74) / 8))
    bar_h = 12
    for i in range(8):
        draw_value_bar(
            canvas,
            ...
            0.0,
            1.0,
            navy if i < 4 else teal,
            f"F{i + 1}",
        )

    imu_y = flex_y + flex_h + gap
    left_acc = float(np.linalg.norm(sensor_vec[8:11]))
    left_gyro = float(np.linalg.norm(sensor_vec[11:14]))
    right_acc = float(np.linalg.norm(sensor_vec[17:20]))
    right_gyro = float(np.linalg.norm(sensor_vec[20:23]))
    cv2.rectangle(canvas, (sx, imu_y), (sx + panel_w, imu_y + imu_h), panel, -1)
    cv2.rectangle(canvas, (sx, imu_y), (sx + 8, imu_y + imu_h), navy, -1)
    cv2.rectangle(canvas, (sx, imu_y), (sx + panel_w, imu_y + imu_h), line, 1)
    cv2.putText(canvas, "IMU", (sx + 18, imu_y + 28), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (16, 20, 26), 2, cv2.LINE_AA)
    imu_bar_y = imu_y + 50
    imu_gap = 23
    draw_value_bar(canvas, sx + 18, imu_bar_y, panel_w - 36, bar_h, left_acc, 0.0, 4.0, navy, "L ACC")
    draw_value_bar(canvas, sx + 18, imu_bar_y + imu_gap, panel_w - 36, bar_h, left_gyro, 0.0, 500.0, navy, "L GYR")
    draw_value_bar(canvas, sx + 18, imu_bar_y + imu_gap * 2, panel_w - 36, bar_h, right_acc, 0.0, 4.0, teal, "R ACC")
    draw_value_bar(canvas, sx + 18, imu_bar_y + imu_gap * 3, panel_w - 36, bar_h, right_gyro, 0.0, 500.0, teal, "R GYR")

    cv2.putText(canvas, "S START  C RESET  Q QUIT", (sx, DASHBOARD_HEIGHT - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (75, 80, 92), 1, cv2.LINE_AA)
    return canvas


def draw_nsu_mark(canvas, center, radius, text_scale=1.0):
    cx, cy = center
    navy = (146, 71, 8)
    teal = (178, 157, 0)
    mint = (94, 190, 176)
    silver = (210, 214, 219)
    cv2.circle(canvas, center, radius, silver, -1, cv2.LINE_AA)
    cv2.circle(canvas, center, int(radius * 0.94), navy, -1, cv2.LINE_AA)
    cv2.circle(canvas, center, int(radius * 0.68), silver, -1, cv2.LINE_AA)
    cv2.circle(canvas, center, int(radius * 0.60), (255, 255, 255), -1, cv2.LINE_AA)

    font_size = int(radius * 0.55 * text_scale)
    draw_text_unicode(canvas, "S", (cx - int(radius * 0.43), cy - int(radius * 0.08)), font_size=font_size, text_color=teal)
    draw_text_unicode(canvas, "N", (cx - int(radius * 0.14), cy - int(radius * 0.31)), font_size=font_size, text_color=navy)
    draw_text_unicode(canvas, "U", (cx + int(radius * 0.16), cy - int(radius * 0.08)), font_size=font_size, text_color=mint)

    cv2.putText(canvas, "NAMSEOUL UNIVERSITY", (cx - int(radius * 0.68), cy - int(radius * 0.75)), cv2.FONT_HERSHEY_SIMPLEX, 0.44 * text_scale, (255, 255, 255), 2, cv2.LINE_AA)
    draw_text_unicode(canvas, "남서울대학교 1994", (cx - int(radius * 0.50), cy + int(radius * 0.72)), font_size=int(radius * 0.14 * text_scale), text_color=(255, 255, 255))
    for ang in [170, 190, 210, 330, 350, 10]:
        rad = np.deg2rad(ang)
        px = int(cx + np.cos(rad) * radius * 0.78)
        py = int(cy + np.sin(rad) * radius * 0.78)
        cv2.rectangle(canvas, (px - 4, py - 4), (px + 4, py + 4), mint if ang % 40 else teal, -1)


def render_intro_splash(splash_img=None, logo_img=None):
    if splash_img is not None:
        return fit_frame_to_cover(splash_img, DASHBOARD_WIDTH, DASHBOARD_HEIGHT)

    navy = (146, 71, 8)
    teal = (178, 157, 0)
    canvas = np.full((DASHBOARD_HEIGHT, DASHBOARD_WIDTH, 3), (245, 249, 252), dtype=np.uint8)
    cv2.rectangle(canvas, (0, 0), (DASHBOARD_WIDTH, 14), navy, -1)
    cv2.rectangle(canvas, (0, 14), (DASHBOARD_WIDTH, 22), teal, -1)
    cv2.rectangle(canvas, (0, DASHBOARD_HEIGHT - 22), (DASHBOARD_WIDTH, DASHBOARD_HEIGHT), navy, -1)
    if logo_img is not None:
        logo_box = int(min(DASHBOARD_WIDTH, DASHBOARD_HEIGHT) * 0.45)
        fitted_logo = fit_frame_to_box(logo_img, logo_box, logo_box)
        fh, fw = fitted_logo.shape[:2]
        lx = (DASHBOARD_WIDTH - fw) // 2
        ly = (DASHBOARD_HEIGHT - fh) // 2 - 54
        cv2.circle(canvas, (DASHBOARD_WIDTH // 2, ly + fh // 2), max(fw, fh) // 2 + 26, (255, 255, 255), -1, cv2.LINE_AA)
        canvas[ly:ly + fh, lx:lx + fw] = fitted_logo
    cv2.putText(canvas, "REAL-TIME SIGN LANGUAGE RECOGNITION", (DASHBOARD_WIDTH // 2 - 275, DASHBOARD_HEIGHT - 96), cv2.FONT_HERSHEY_SIMPLEX, 0.78, navy, 2, cv2.LINE_AA)
    cv2.line(canvas, (DASHBOARD_WIDTH // 2 - 220, DASHBOARD_HEIGHT - 78), (DASHBOARD_WIDTH // 2 + 220, DASHBOARD_HEIGHT - 78), teal, 3)
    cv2.putText(canvas, "Fusion + Vision Demo", (DASHBOARD_WIDTH // 2 - 118, DASHBOARD_HEIGHT - 54), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (70, 80, 92), 2, cv2.LINE_AA)
    return canvas


def render_transition_screen(title, subtitle="", remain_text="", progress=0.0, logo_img=None, camera_img=None):
    canvas = np.full((DASHBOARD_HEIGHT, DASHBOARD_WIDTH, 3), (245, 249, 252), dtype=np.uint8)
    left_w = int(DASHBOARD_WIDTH * 0.38)
    cv2.rectangle(canvas, (0, 0), (left_w, DASHBOARD_HEIGHT), (146, 71, 8), -1)
    cv2.rectangle(canvas, (left_w - 18, 0), (left_w, DASHBOARD_HEIGHT), (178, 157, 0), -1)
    cv2.rectangle(canvas, (left_w, 0), (DASHBOARD_WIDTH, DASHBOARD_HEIGHT), (245, 249, 252), -1)
    cv2.rectangle(canvas, (left_w, 0), (DASHBOARD_WIDTH, 12), (146, 71, 8), -1)
    cv2.rectangle(canvas, (left_w, 12), (DASHBOARD_WIDTH, 20), (178, 157, 0), -1)

    if logo_img is not None:
        logo_size = min(left_w - 130, DASHBOARD_HEIGHT - 190)
        fitted = fit_frame_to_box(logo_img, logo_size, logo_size)
        fh, fw = fitted.shape[:2]
        lx = (left_w - fw) // 2
        ly = (DASHBOARD_HEIGHT - fh) // 2 - 18
        cv2.circle(canvas, (left_w // 2, ly + fh // 2), max(fw, fh) // 2 + 24, (255, 255, 255), -1, cv2.LINE_AA)
        canvas[ly:ly + fh, lx:lx + fw] = fitted
    else:
        draw_nsu_mark(canvas, (left_w // 2, DASHBOARD_HEIGHT // 2 - 24), min(185, left_w // 3), text_scale=1.0)

    cv2.putText(canvas, "NAMSEOUL UNIVERSITY", (54, DASHBOARD_HEIGHT - 80), cv2.FONT_HERSHEY_SIMPLEX, 0.78, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(canvas, "REAL-TIME SIGN LANGUAGE RECOGNITION", (54, DASHBOARD_HEIGHT - 46), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (205, 230, 255), 1, cv2.LINE_AA)

    rx = left_w + 64
    cv2.rectangle(canvas, (rx, 82), (rx + 112, 118), (146, 71, 8), -1)
    cv2.putText(canvas, "NSU", (rx + 28, 107), cv2.FONT_HERSHEY_SIMPLEX, 0.72, (255, 255, 255), 2, cv2.LINE_AA)
    text_max_w = DASHBOARD_WIDTH - rx - 90
    draw_text_unicode_fit(canvas, title, (rx, 138), text_max_w, font_size=46, min_font_size=26, text_color=(16, 20, 26))
    if subtitle:
        draw_text_unicode_fit(canvas, subtitle, (rx, 204), text_max_w, font_size=29, min_font_size=20, text_color=(76, 84, 96))
    if remain_text:
        cv2.putText(canvas, remain_text, (rx, 292), cv2.FONT_HERSHEY_SIMPLEX, 1.58, (146, 71, 8), 4, cv2.LINE_AA)

    bar_x, bar_y, bar_w, bar_h = rx, 342, DASHBOARD_WIDTH - rx - 88, 18
    cv2.rectangle(canvas, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), (218, 224, 232), -1)
    fill_w = int(bar_w * float(np.clip(progress, 0.0, 1.0)))
    cv2.rectangle(canvas, (bar_x, bar_y), (bar_x + fill_w, bar_y + bar_h), (178, 157, 0), -1)
    cv2.rectangle(canvas, (bar_x, bar_y), (bar_x + bar_w, bar_y + bar_h), (180, 188, 198), 1)

    if camera_img is not None:
        preview_w, preview_h = 360, 220
        fitted = fit_frame_to_box(camera_img, preview_w, preview_h)
        fh, fw = fitted.shape[:2]
        px, py = rx, DASHBOARD_HEIGHT - preview_h - 70
        cv2.rectangle(canvas, (px, py), (px + preview_w, py + preview_h), (16, 20, 26), -1)
        canvas[py + (preview_h - fh) // 2:py + (preview_h - fh) // 2 + fh, px + (preview_w - fw) // 2:px + (preview_w - fw) // 2 + fw] = fitted
        cv2.rectangle(canvas, (px, py), (px + preview_w, py + preview_h), (185, 192, 204), 1)

    cv2.putText(canvas, "Q: QUIT", (DASHBOARD_WIDTH - 170, DASHBOARD_HEIGHT - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (80, 88, 100), 2, cv2.LINE_AA)
    return canvas
...
def show_startup_calibration_guide():
    ...
    for title, subtitle, duration_sec, bar_color in STARTUP_GUIDE_STAGES:
        ...
        while True:
            ...
            progress = float(np.clip(elapsed / max(duration_sec, 1e-6), 0.0, 1.0))
            ...
            guide_img = render_transition_screen(
                title=title,
                subtitle=subtitle,
...
def show_wait_sensor_screen():
    ...
    while True:
        ...
        img = cv2.flip(img, 1)
        wait_img = render_transition_screen(
            title="센서 연결 확인 중",
            subtitle="장갑 센서 패킷을 기다리고 있습니다",
            remain_text="WAITING",
            progress=min(1.0, (time.time() - start) / 12.0),
            logo_img=logo_img,
            camera_img=img,
        )
...
def show_intro_splash():
    splash = render_intro_splash(logo_img=UI_LOGO_IMG)
    start = time.time()
    while time.time() - start < INTRO_SPLASH_SEC:
...
@tf.function
def infer_vision_only(vision_input_tensor):
    return vision_model(vision_input_tensor, training=False)
...
if VISION_COMPARE_ENABLED and vision_model is not None:
    _ = infer_vision_only(tf.convert_to_tensor(dummy_vision))

UI_LOGO_IMG = load_nsu_logo()
intro_result = show_intro_splash()
...
current_vision_result = make_empty_prediction()
current_vision_result['enabled'] = VISION_COMPARE_ENABLED
...
print(
    f"[TTS] enabled={TTS_ENABLED} min_interval={TTS_MIN_INTERVAL_SEC:.1f}s "
    f"skip_none={TTS_SKIP_NONE} backend={TTS_BACKEND}"
)
...
def make_pyttsx3_engine():
    import pyttsx3

    engine = pyttsx3.init()
    engine.setProperty("rate", 165)
    engine.setProperty("volume", max(0.0, min(1.0, TTS_VOLUME / 100.0)))
    return engine


edge_tts_cache = {}
edge_tts_executable = None


def make_edge_cache_key(text):
    return f"{TTS_EDGE_VOICE}|{TTS_EDGE_RATE}|{text}"


def find_edge_tts_executable():
    if TTS_EDGE_EXE and os.path.exists(TTS_EDGE_EXE):
        return TTS_EDGE_EXE

    found = shutil.which("edge-tts") or shutil.which("edge-tts.exe")
    if found:
        return found

    local_app_data = os.getenv("LOCALAPPDATA")
    if local_app_data:
        candidates = pathlib.Path(local_app_data).glob("Python/pythoncore-*/Scripts/edge-tts.exe")
...
def ensure_edge_tts_available():
    global edge_tts_executable
    try:
        import edge_tts  # noqa: F401
        return "python"
    except Exception:
        edge_tts_executable = find_edge_tts_executable()
        if edge_tts_executable:
            return edge_tts_executable
        raise


def get_edge_tts_path(text):
    clean_text = str(text).strip()
    if not clean_text:
        return None
    cache_key = make_edge_cache_key(clean_text)
    if cache_key in edge_tts_cache and os.path.exists(edge_tts_cache[cache_key]):
        return edge_tts_cache[cache_key]

    safe_name = base64.urlsafe_b64encode(cache_key.encode("utf-8")).decode("ascii").rstrip("=")
    output_path = os.path.join(tempfile.gettempdir(), f"nsu_edge_tts_{safe_name}.mp3")
    if os.path.exists(output_path):
        edge_tts_cache[cache_key] = output_path
        return output_path
    ...
    try:
        ...
        async def save_audio():
            communicate = edge_tts.Communicate(clean_text, TTS_EDGE_VOICE, rate=TTS_EDGE_RATE)
            await communicate.save(output_path)

        asyncio.run(save_audio())
    except Exception as import_or_generate_error:
        exe_path = find_edge_tts_executable()
        if not exe_path:
            raise import_or_generate_error
        ...
        if result.returncode != 0:
            raise RuntimeError((result.stderr or "").strip() or f"edge-tts failed with code {result.returncode}")

    edge_tts_cache[cache_key] = output_path
    if TTS_DEBUG:
        print(f"[TTS] Edge generated text={clean_text} voice={TTS_EDGE_VOICE} rate={TTS_EDGE_RATE}")
    return output_path


def make_edge_audio_player_process():
    command = (
        "[Console]::InputEncoding = [System.Text.UTF8Encoding]::new($false); "
        "Add-Type -AssemblyName presentationCore; "
        "$player = New-Object System.Windows.Media.MediaPlayer; "
        "while (($line = [Console]::In.ReadLine()) -ne $null) { "
        "if ($line -eq '__NSU_TTS_STOP__') { break }; "
        "$uri = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($line)); "
        "$player.Stop(); "
        "$player.Open([Uri]::new($uri)); "
        "$player.Play(); "
        "while ($player.NaturalDuration.HasTimeSpan -eq $false) { Start-Sleep -Milliseconds 5 }; "
        "$duration = $player.NaturalDuration.TimeSpan.TotalMilliseconds; "
        f"Start-Sleep -Milliseconds ([Math]::Ceiling($duration) + {max(0, TTS_EDGE_EXTRA_SLEEP_MS)}); "
        "}; "
        "$player.Close()"
    )
...
def stop_edge_audio_player_process(process):
    if process is None:
        return
    try:
        if process.stdin:
            process.stdin.write("__NSU_TTS_STOP__\n")
            process.stdin.flush()
    except Exception:
        pass
    try:
...
def play_audio_file_windows(path, process=None):
    file_uri = pathlib.Path(path).resolve().as_uri()
    if process is not None and process.poll() is None and process.stdin is not None:
        encoded_uri = base64.b64encode(file_uri.encode("utf-8")).decode("ascii")
        process.stdin.write(encoded_uri + "\n")
        process.stdin.flush()
        return
    ...
    command = (
        "Add-Type -AssemblyName presentationCore; "
        "$player = New-Object System.Windows.Media.MediaPlayer; "
        f"$player.Open([Uri]::new({json.dumps(file_uri)})); "
        "$player.Play(); "
...
def speak_with_edge_tts(text, player_process=None):
    audio_path = get_edge_tts_path(text)
    if audio_path:
        play_audio_file_windows(audio_path, player_process)


def precache_edge_tts_words():
    if not TTS_EDGE_PRECACHE:
        return
    words = sorted({value for key, value in KOR_MAP.items() if key != 'none'})
    for word in words:
        get_edge_tts_path(word)
    if TTS_DEBUG:
        print(f"[TTS] Edge cached {len(words)} Korean words")


def make_windows_sapi_process():
    command = (
        "[Console]::InputEncoding = [System.Text.UTF8Encoding]::new($false); "
...
def stop_windows_sapi_process(process):
    if process is None:
        return
    try:
        if process.stdin:
...
def tts_worker():
    engine = None
    sapi_process = None
    edge_player_process = None
    backend = TTS_BACKEND

    if backend not in ("sapi", "pyttsx3", "edge"):
        print(f"[TTS] unknown backend '{TTS_BACKEND}', using sapi")
        backend = "sapi"

    if backend == "edge":
        try:
            edge_source = ensure_edge_tts_available()
            precache_edge_tts_words()
            edge_player_process = make_edge_audio_player_process()
            if TTS_DEBUG:
                print(f"[TTS] Edge neural backend ready source={edge_source} voice={TTS_EDGE_VOICE} rate={TTS_EDGE_RATE}")
        except Exception as e:
            print(f"[TTS] edge-tts unavailable: {e}")
            if TTS_EDGE_REQUIRE:
                print("[TTS] Edge is required, disabling TTS instead of falling back to SAPI")
                return
    ...
    if backend == "pyttsx3":
        try:
            engine = make_pyttsx3_engine()
            print("[TTS] pyttsx3 backend ready")
        except Exception as e:
            print(f"[TTS] pyttsx3 unavailable, using Windows SAPI fallback: {e}")
            backend = "sapi"
    if backend == "sapi":
        try:
            sapi_process = make_windows_sapi_process()
            if TTS_DEBUG:
                print("[TTS] Windows SAPI backend ready")
        except Exception as e:
            print(f"[TTS] persistent Windows SAPI unavailable, using per-call fallback: {e}")
    ...
    while not tts_stop_event.is_set():
        ...
        try:
            ...
            if backend == "edge":
                speak_with_edge_tts(text, edge_player_process)
            elif backend == "pyttsx3" and engine is not None:
                engine.say(text)
                engine.runAndWait()
            else:
                speak_with_windows_sapi(text, sapi_process)
        except Exception as e:
            print(f"[TTS] speak failed: {e}")

    stop_windows_sapi_process(sapi_process)
    stop_edge_audio_player_process(edge_player_process)


def speak_with_windows_sapi(text, process=None):
    if process is not None and process.poll() is None and process.stdin is not None:
    ...
    if result.returncode != 0:
        ...
        print(f"[TTS] Windows SAPI failed: code={result.returncode} {err}")


def tts_speak_async(label, text):
    global tts_last_label, tts_last_time

    if not TTS_ENABLED:
        return
    if TTS_SKIP_NONE and label == 'none':
        return

    now = time.time()
    if label == tts_last_label and (now - tts_last_time) < TTS_MIN_INTERVAL_SEC:
        return

    tts_last_label = label
    tts_last_time = now
    try:
        while True:
            try:
        ...
        if TTS_DEBUG:
            print(f"[TTS] queued: label={label} text={text}")
    except queue.Full:
        print(f"[TTS] queue full, skipped: {text}")
...
def save_debug_capture(seq_array, result, frame_count, sensor_time_ms, sensor_host_delta_ms):
    ...
    meta = {
        ...
        "margin": float(result.get("margin", 0.0)),
        "vision_compare_enabled": bool(result.get("vision", {}).get("enabled", False)),
        "vision_raw_label": result.get("vision", {}).get("raw_label"),
        "vision_raw_conf": float(result.get("vision", {}).get("raw_conf", 0.0)),
        "vision_label": result.get("vision", {}).get("label"),
        "vision_conf": float(result.get("vision", {}).get("conf", 0.0)),
        "vision_margin": float(result.get("vision", {}).get("margin", 0.0)),
        "vision_top_candidates": result.get("vision", {}).get("top_candidates", []),
        "vision_model_dir": DEFAULT_VISION_MODEL_DIR,
...
try:
    while True:
        ...
        if mode == "RECORDING":
            ...
            if no_hand_run >= NO_HAND_RESET_FRAMES:
                ...
                current_vision_result = result.get('vision', make_empty_prediction())
                long_motion_score = result['long_motion']
                last_margin = result['margin']

                kor_text = get_kor_label_text(stable_label)
                tts_speak_async(stable_label, kor_text)
                vision_text = get_kor_label_text(current_vision_result.get('label', 'none'))
                print(
                    f"[Result] stable={stable_label} ({kor_text}) | conf={stable_conf:.3f} | "
                    f"margin={last_margin:.3f} | motion={long_motion_score:.3f}"
                )
                if current_vision_result.get('enabled', False):
                    print(
                        f"[VisionOnly] label={current_vision_result.get('label')} ({vision_text}) | "
                        f"conf={float(current_vision_result.get('conf', 0.0)):.3f} | "
                        f"margin={float(current_vision_result.get('margin', 0.0)):.3f}"
                    )
        ...
        showcase_img = render_showcase_layout(
            ...
            vision_result=current_vision_result,
            sensor_vec=sensor_vec,
        ...
        if SHOW_DEBUG_OVERLAY:
            debug_lines = [
                ...
                f"VisionOnly={current_vision_result.get('label', 'none')} conf={float(current_vision_result.get('conf', 0.0)):.3f}",
            ...
            draw_debug_panel(showcase_img, debug_lines)

        cv2.imshow(WINDOW_NAME, showcase_img)
    ...
    profile_path = runtime_profiler.save(
        ...
        extra={
            "model_path": MODEL_PATH,
            "save_debug_capture": SAVE_DEBUG_CAPTURE,
